...
export const sendSignUpEmail = inngest.createFunction(
    ...
    async ({event, step}) => {
        ...
        await step.run('send-welcome-email', async () => {
            ...
            const introText = (part && 'text' in part ? part.text : null) || 'Thanks for joinning Signalist.You now have the tools to track markets and make smarter moves.'
...
export const sendDailyNewsSummary = inngest.createFunction(
    ...
    [{event: 'app/send.daily.news'}, {cron: '0 12 * * *'}],  // min hour  dayofMonth month dayofweek , 0 12 * * * => hour 12 every daily at 12:00 UTC
    async ({step}) => {
